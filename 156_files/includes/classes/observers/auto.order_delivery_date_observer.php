...
class zcObserverOrderDeliveryDateObserver extends base {
  ...
  // ZC 1.5.6 - 1.5.7: $zco_notifier->notify('NOTIFY_ADMIN_ORDERS_LIST_EXTRA_COLUMN_DATA', (isset($oInfo) ? $oInfo : array()), $orders->fields, $extra_data);
  function updateNotifyAdminOrdersListExtraColumnData(&$callingClass, $notifier, $oInfo_cond, &$orders_fields, &$extra_data) {
    ...
    if (!array_key_exists('order_delivery_date', $orders_fields)) {
      global $sniffer, $db;

      if ($sniffer->field_exists(TABLE_ORDERS, 'order_delivery_date')) {
        $sql = "SELECT order_delivery_date FROM " . TABLE_ORDERS . "
                  WHERE orders_id = " . (int)($orders_fields['orders_id'] . " LIMIT 1";

        $order_delivery_date = $db->Execute($sql);
      }

      $orders_fields['order_delivery_date'] = (isset($order_delivery_date) && isset($order_delivery_date->fields['order_delivery_date'])) ? $order_delivery_date->fields['order_delivery_date'] : null;
    }
