...
function zen_update_orders_history($orders_id, $message = '', $updated_by = null, $orders_new_status = -1, $notify_customer = -1, $email_include_message = true, $email_subject = '', $send_extra_emails_to = '') 
{
    ...
    $osh_info = $GLOBALS['db']->Execute(
        "SELECT customers_name, customers_email_address, orders_status, date_purchased, order_delivery_date 
           FROM " . TABLE_ORDERS . " 
    ...
    if ($osh_info->EOF) {
        ...
        $email_message =  EMAIL_TEXT_DELIVERY_DATE . ' ' . zen_date_long($osh_info->fields['order_delivery_date']) . "\n\n";
        ...
        if (($orders_new_status != -1 && $orders_current_status != $orders_new_status) || !empty($email_message)) {
            ...
            if ($notify_customer == 1 || $notify_customer == -2) {
                ...
                $html_msg['EMAIL_TEXT_DELIVERY_DATE'] = EMAIL_TEXT_DELIVERY_DATE . ' ' . zen_date_long($osh_info->fields['order_delivery_date']);
